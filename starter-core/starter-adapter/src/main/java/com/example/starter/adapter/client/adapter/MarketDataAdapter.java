...
class MarketDataAdapter implements MarketDataPort {
    ...
    public Optional<BigDecimal> currentPrice(String isin) {
        try {
            QuoteResponse quote =
                    restClient.get().uri("/quotes/{isin}", isin).retrieve().body(QuoteResponse.class);
            return Optional.ofNullable(quote).map(QuoteResponse::price);
    ...
    record QuoteResponse(String isin, BigDecimal price) {}
}
